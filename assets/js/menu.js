...
/*====================opMenu=====================*/
const wrapper = document.querySelector(".menuWrapper");
...
document.querySelector('header #menu').addEventListener('click', function() {
    ...
    htmlElement.classList.toggle('opMenu');
    if (htmlElement.classList.contains('opMenu')) {
        lenis.stop();
        if (isMobile.any) {
        ...
        document.getElementById('menu-container').addEventListener('wheel', function(e) {
...
/*====================MENU=====================*/
/*====================CURSOR=====================*/
const mouse = { x: undefined, y: undefined, cX: undefined, cY:undefined };
document.addEventListener("pointermove", (event) => {
    mouse.x = Math.floor(event.clientX);
    mouse.y = Math.floor(event.clientY);
    document.querySelectorAll('#coordinate-x > *')[0].textContent = mouse.x;
    document.querySelectorAll('#coordinate-y > *')[0].textContent = mouse.y;
    document.querySelectorAll('.simultaneous > *').forEach((element) => {
        element.style.transform = `translate(${mouse.x}px, ${mouse.y}px)`;
    });
    document.querySelectorAll('.simultaneousC').forEach((element) => {
        element.style.transform = `translate(${mouse.x + 10}px, ${mouse.y + 10}px)`;
    });
    document.querySelectorAll('.coordinate-square').forEach((element) => {
        element.style.transform = `translate(${mouse.x}px, ${mouse.y}px)`;
    });
    document.querySelectorAll('.coordinate-line').forEach((element) => {
        element.style.transform = `translate(${mouse.x}px, ${mouse.y}px)`;
    });
});
let aElements = document.getElementsByTagName('a');
for (let i = 0; i < aElements.length; i++) {
    aElements[i].classList.add('hover-target');
}
let buttonElements = document.getElementsByTagName('button');
for (let i = 0; i < buttonElements.length; i++) {
    buttonElements[i].classList.add('hover-target');
}
let hoverTargets = document.getElementsByClassName('hover-target');
for (let i = 0; i < hoverTargets.length; i++) {
    hoverTargets[i].addEventListener('mouseenter', function () {
        var coordinateBoxes = document.getElementsByClassName('coordinate-box');
        for (let j = 0; j < coordinateBoxes.length; j++) {
            coordinateBoxes[j].classList.add('hover');
        }
    });
    hoverTargets[i].addEventListener('mouseleave', function () {
        var coordinateBoxes = document.getElementsByClassName('coordinate-box');
        for (let j = 0; j < coordinateBoxes.length; j++) {
            coordinateBoxes[j].classList.remove('hover');
        }
    });
}
/*====================CURSOR=====================*/